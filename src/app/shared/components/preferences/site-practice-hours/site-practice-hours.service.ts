...
export class SitePracticeHoursService {
  ...
  public getCurrentSitePracticeHours(): Observable<WorkDayInputModel[]> {
    const url = `${environment.baseurl}/sitePracticeHours` ;
    return this.http.get<WorkDayInputModel[]>(url);
  }

  public setSitePracticeHorus(model: any): Observable<ResponseModel> {
    const url = `${environment.baseurl}/sitePracticeHours`;
